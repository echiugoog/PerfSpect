...
func storagePerfFromOutput(outputs map[string]script.ScriptOutput) (readIOPS, readLat, readBw, writeIOPS, writeLat, writeBw string) {
	output := outputs[script.StorageBenchmarkScriptName].Stdout
	i := strings.Index(output, "{\n  \"fio version\"")
	if i >= 0 {
		output = output[i:]
	} else {
		slog.Warn("Unable to find fio output")
	}
	...
	slog.Info("parsing storage benchmark output")
	...
	if len(fioData.Jobs) > 0 {
		slog.Info("jobs found in storage benchmark output")
		job := fioData.Jobs[0]
		readBw = fmt.Sprintf("%d", job.Read.Bw)
		readIOPS = fmt.Sprintf("%.0f", job.Read.Iops)
		readLat = fmt.Sprintf("%.0f", job.Read.LatNs.Mean)
		writeBw = fmt.Sprintf("%d", job.Write.Bw)
		writeIOPS = fmt.Sprintf("%.0f", job.Write.Iops)
	...
	slog.Info("fini: storage benchmark output", slog.String("readIOPS", readIOPS), slog.String("readLat", readLat), slog.String("readBw", readBw), slog.String("writeIOPS", writeIOPS), slog.String("writeLat", writeLat), slog.String("writeBw", writeBw))
	return
}
